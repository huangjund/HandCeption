...
# load pointcloud from camera and translate to blender's world coordinate
...
L2 = trimesh.load("YCB_Video_Dataset/YCB_Video_Models/models/001_L2/L2.obj")
L3 = trimesh.load("YCB_Video_Dataset/YCB_Video_Models/models/002_L3/L3.obj")
R2 = trimesh.load("YCB_Video_Dataset/YCB_Video_Models/models/003_R2/R2.obj")
R3 = trimesh.load("YCB_Video_Dataset/YCB_Video_Models/models/004_R3/R3.obj")
base_link_kps = np.loadtxt('001_base_link_8_kps.txt')
L2_kps = np.loadtxt('ycb_kps/001_L2_8_kps.txt')
L3_kps = np.loadtxt('ycb_kps/002_L3_8_kps.txt')
R2_kps = np.loadtxt('ycb_kps/003_R2_8_kps.txt')
R3_kps = np.loadtxt('ycb_kps/004_R3_8_kps.txt')
...
L2_kps = (RTs[0,:,:3] @ L2_kps.T + RTs[0,:,3].reshape(3,1)).T
#L3_kps = (rot_x(90) @ L3_kps.T).T
L3_kps = (RTs[1,:,:3] @ L3_kps.T + RTs[1,:,3].reshape(3,1)).T
#R2_kps = (rot_x(90) @ R2_kps.T).T
R2_kps = (RTs[2,:,:3] @ R2_kps.T + RTs[2,:,3].reshape(3,1)).T
#R3_kps = (rot_x(90) @ R3_kps.T).T
R3_kps = (RTs[3,:,:3] @ R3_kps.T + RTs[3,:,3].reshape(3,1)).T
...
L2_2position = (RTs[0,:,:3] @ L2_cld_origin.T + RTs[0,:,3].reshape(3,1)).T
L3_2position = (RTs[1,:,:3] @ L3_cld_origin.T + RTs[1,:,3].reshape(3,1)).T
R2_2position = (RTs[2,:,:3] @ R2_cld_origin.T + RTs[2,:,3].reshape(3,1)).T
R3_2position = (RTs[3,:,:3] @ R3_cld_origin.T + RTs[3,:,3].reshape(3,1)).T
...
ax.scatter(cld_inworldframe[:,0],cld_inworldframe[:,1],cld_inworldframe[:,2],c=colors[6], s=4)

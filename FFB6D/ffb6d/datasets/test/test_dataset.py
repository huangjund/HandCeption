...
class Dataset():
    ...
    def __init__(self, dataset_name, DEBUG=False):
        ...
        self.cls_lst = bs_utils.read_lines(config.test_cls_lst_p)
        self.obj_dict = {}
        ...
        if dataset_name == 'train':
            self.add_noise = True
            self.path = 'datasets/test/dataset_config/train_data_list.txt'
            ...
            self.path = 'datasets/test/dataset_config/test_data_list.txt'
            self.all_lst = bs_utils.read_lines(self.path)
        print("{}_dataset_size: ".format(dataset_name), len(self.all_lst))
        self.root = config.test_root
    ...
    def get_item(self, item_name):
        ...
        if item_name[:8] != 'data_syn' and int(item_name[5:9]) >= 60:
            K = config.intrinsic_matrix['test_K2']
        else:
            K = config.intrinsic_matrix['test_K1']
    ...
    def get_pose_gt_info(self, cld, labels, cls_id_lst, meta):
        ...
        for i, cls_id in enumerate(cls_id_lst):
            ...
            kps = bs_utils.get_kps(
                self.cls_lst[cls_id-1], kp_type=kp_type, ds_type='test'
            ).copy()
